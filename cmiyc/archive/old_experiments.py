...
class Experiment():
    ...
    def load_data(self):
        '''
        Loads the data using the dataset_utils function and encodes the signatures.
        '''
        # Load data
        ...
        x_encoded = self.vanilla_vae.encoder.predict(x)

        # Get the losses
        history = LossHistory()
        self.vanilla_vae.vae.fit(x, epochs=1, batch_size=1, callbacks=[history]) # use vanilla_vae.vae.predict?
        self.losses = np.asarray(history.losses).reshape(-1, 1)

        # Split data
        # x_train, x_test, y_train, y_test = train_test_split(x_encoded, y, test_size=0.2) # Train using latent
        x_train, x_test, y_train, y_test = train_test_split(self.losses, y, test_size=0.2) # Train using loss
    ...
    def plot_AUC(self, fpr_keras, tpr_keras, auc_keras, classifier, save_img=False):
        '''
        Plots the AUC and saves the file to experiment folder
        '''
        plt.figure(1)
        plt.plot([0, 1], [0, 1], 'k--')
